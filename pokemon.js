...
function generatePokemonCode() {
	...
	const pokemonIndex = parseInt(pokemonIndexDecimal)
		.toString(16)
		.toUpperCase()
		.padStart(4, "0");
...
function generatePokemonBoxCode(hex) {
	...
	boxCodes.push({
		boxName: "Box 3",
		boxCode: `(EE${box3Char}B!n[space][space])`,
	});
	...
	boxCodes.push({
		boxName: "Box 4",
		boxCode: `(E${box4Char}/!n[space][space][space])`,
	});
	...
	boxCodes.push({ boxName: "Box 7", boxCode: `(EED…Rm[space][space])` });

	boxCodes.push({
		boxName: "Box 8",
		boxCode: `(E[space][space][space][space][space][space][space])`,
	});
	...
	boxCodes.push({
		boxName: "Box 10",
		boxCode: `([space][space][space][space][space]…oa)`,
	});

	boxCodes.push({
		boxName: "Box 11",
		boxCode: `(…o[space][space][space][space][space][space])`,
	});
...
function getBox4Char(second, third, first) {
	...
	const map =
		"DEF".includes(second) || (second === "C" && "CDEF".includes(third))
			? mapSpecial
			: mapNormal;
